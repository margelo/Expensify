...
The release build workflows now contain a candidate CI path for Android and iOS. Release jobs select dedicated `pgo-android` and `pgo-ios` physical-device runners; AdHoc builds retain their existing hosted runners. The same job builds and installs an instrumented app, verifies the signed-in heavy account, turns off #focus through the journey preflight, records three complete journeys, validates the merged profile, and then asks Rock for a local source-based store build that consumes it. The build fails if the fixture, device, profile, or app artifact is missing. A small proof artifact binds the resulting AAB or IPA hash to the profile hash and both repository revisions; it contains no account identity or journey data.
